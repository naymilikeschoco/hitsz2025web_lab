...
void tcp_out(tcp_conn_t *tcp_conn, buf_t *buf, uint16_t src_port, uint8_t *dst_ip, uint16_t dst_port, uint8_t flags) {
    ...
    hdr->uptr = 0;
    hdr->doff = ((sizeof(tcp_hdr_t) / 4) << 4);  // 首部长度，以4字节为单位
    // hdr->doff = (0x50) & 0xF0;  // 0x50 = 0101 0000，表示头部长度为5（5*4=20字节）
    hdr->flags = flags;
...
void tcp_in(buf_t *buf, uint8_t *src_ip) {
    ...
    // TODO
    //调用map_get()函数查询tcp_handler_table是否有该目的端口号对应的处理函数（回调函数）。
    //如果没有找到，则调用buf_add_header()函数增加IPv4数据报头部，
    //再调用icmp_unreachable()函数发送一个端口不可达的ICMP差错报文。
    //如果能找到，则去掉TCP报头，调用处理函数来做相应处理。
    ...
    if (payload_len > 0) {
        ...
        if (handler && *handler) {
            ...
            (*handler)(tcp_conn, buf->data, payload_len, remote_ip, remote_port);
        }
        else{
            // 发送端口不可达的ICMP差错报文
